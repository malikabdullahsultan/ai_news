...
function architectureSummaryBody() {
  return `<section class="architecture-hero page-container" data-reveal><div class="architecture-hero-copy"><div class="eyebrow eyebrow-pill"><span class="pulse-dot"></span> SYSTEM MAP · KID-FRIENDLY EDITION</div><h1>How this little AI newsroom works.</h1><p>Imagine a tiny newsroom, a careful librarian, and a fast printing press working together. This page explains the journey in plain language—without showing any private keys or secret settings.</p><div class="architecture-actions"><a class="button button-primary" href="${urlFor('latest/')}">See the finished report <span>→</span></a><a class="button button-ghost" href="${urlFor('archive/')}">Browse the shelves</a></div></div><div class="architecture-orbit" aria-label="A diagram showing research flowing into a report"><div class="architecture-orbit-ring ring-one"></div><div class="architecture-orbit-ring ring-two"></div><div class="architecture-orbit-core"><span>AI</span><small>NEWSROOM</small></div><span class="architecture-orbit-chip chip-one">SOURCES</span><span class="architecture-orbit-chip chip-two">CHECKS</span><span class="architecture-orbit-chip chip-three">PAGES</span></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">THE BIG JOURNEY</div><h2>From “what happened?” to “here is what matters.”</h2></div><p>Every daily report takes the same careful route, like a package moving through labeled stations.</p></div><div class="architecture-pipeline"><article class="architecture-step"><div class="architecture-step-number">01</div><div class="architecture-step-icon">🛰️</div><h3>Scouts look around</h3><p>Small research scripts read approved news feeds, papers, and official pages. They collect clues, not passwords.</p><span class="architecture-step-tag">COLLECT</span></article><div class="architecture-arrow" aria-hidden="true">→</div><article class="architecture-step"><div class="architecture-step-number">02</div><div class="architecture-step-icon">🧠</div><h3>A writer finds the thread</h3><p>The report writer turns the clues into a short story, keeping links beside the claims so readers can check them.</p><span class="architecture-step-tag">EXPLAIN</span></article><div class="architecture-arrow" aria-hidden="true">→</div><article class="architecture-step"><div class="architecture-step-number">03</div><div class="architecture-step-icon">🛡️</div><h3>A checker plays referee</h3><p>Validation looks for missing dates, missing sources, broken placeholders, provider errors, and duplicate report days.</p><span class="architecture-step-tag">VERIFY</span></article><div class="architecture-arrow" aria-hidden="true">→</div><article class="architecture-step"><div class="architecture-step-number">04</div><div class="architecture-step-icon">📚</div><h3>The shelves get a new page</h3><p>A build script turns Markdown into fast HTML pages for the latest report, archive, search, and each dated report.</p><span class="architecture-step-tag">PUBLISH</span></article></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">THE MACHINE ROOM</div><h2>Six friendly parts, one calm dashboard.</h2></div><p>These are the jobs—not secret ingredients—that make the site tick.</p></div><div class="architecture-parts"><article class="architecture-part"><span class="architecture-part-icon">⏰</span><div><h3>The daily clock</h3><p>A scheduled workflow starts the research run around the Hong Kong morning target.</p></div></article><article class="architecture-part"><span class="architecture-part-icon">🗺️</span><div><h3>The source map</h3><p>A configuration list tells the scouts which official, research, and discovery sources to watch.</p></div></article><article class="architecture-part"><span class="architecture-part-icon">✍️</span><div><h3>The writing prompt</h3><p>A plain-text recipe gives the writer its tone, sections, evidence rules, and continuity clues.</p></div></article><article class="architecture-part"><span class="architecture-part-icon">🧪</span><div><h3>The safety lab</h3><p>Tests and report checks catch mistakes before a new page is allowed onto the public shelves.</p></div></article><article class="architecture-part"><span class="architecture-part-icon">🧱</span><div><h3>The page builder</h3><p>One static build makes the homepage, report pages, archive cards, and searchable index from saved Markdown.</p></div></article><article class="architecture-part"><span class="architecture-part-icon">🌐</span><div><h3>The browser window</h3><p>Your browser downloads HTML, CSS, and JavaScript. There is no database server waiting behind each click.</p></div></article></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-safety"><div><div class="eyebrow">THE LOCKED DRAWER</div><h2>What stays private?</h2><p>The architecture guide explains the shape of the machine, not the contents of its locked drawer.</p></div><div class="architecture-safety-list"><div><span>🔒</span><p><strong>Secret values</strong><br>Private provider credentials live in the automation platform’s secret storage.</p></div><div><span>🚫</span><p><strong>No surprise overwrite</strong><br>If a date already has a report, the generator stops instead of erasing it.</p></div><div><span>🧯</span><p><strong>Safe failure</strong><br>If research or writing fails, yesterday’s published pages are left alone.</p></div></div></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">TINY GLOSSARY</div><h2>Big words, small explanations.</h2></div></div><div class="architecture-glossary"><div><dt>Markdown</dt><dd>Simple text with headings and links. It is the report’s notebook form.</dd></div><div><dt>Static site</dt><dd>Pages are prepared before you visit, like sandwiches packed before lunch.</dd></div><div><dt>Workflow</dt><dd>A recipe of computer jobs that run in a reliable order.</dd></div><div><dt>Validation</dt><dd>A checklist that says “ready” or “please fix this first.”</dd></div></div></section><section class="architecture-next page-container" data-reveal><span class="architecture-next-spark">✦</span><div><div class="eyebrow">YOUR NEXT STOP</div><h2>Now you know the backstage trick.</h2><p>Read a real briefing, explore the archive, or search the signal.</p></div><div class="architecture-next-links"><a class="button button-light" href="${urlFor('latest/')}">Read latest</a><a class="button button-light" href="${urlFor('search/')}">Search reports</a></div></section>`;
}

function architectureBody() {
  const deepDive = `<section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">ZOOM ALL THE WAY IN</div><h2>A report’s day, step by step.</h2></div><p>The simple four-step journey contains smaller jobs. Here is the full relay race.</p></div><ol class="architecture-timeline"><li><span>01</span><div><strong>Choose the Hong Kong date</strong><p>The clock is converted into the report’s calendar before any file gets its name.</p></div></li><li><span>02</span><div><strong>Open the research window</strong><p>Recent items are welcome; stale items are filtered away.</p></div></li><li><span>03</span><div><strong>Read the public source map</strong><p>The scouts learn which official, research, and discovery shelves to visit.</p></div></li><li><span>04</span><div><strong>Normalize the clues</strong><p>Different feed formats become one tidy shape: title, link, time, source, summary, and topics.</p></div></li><li><span>05</span><div><strong>Cluster duplicate stories</strong><p>Five articles about one launch become one evidence bundle instead of five pretend events.</p></div></li><li><span>06</span><div><strong>Add continuity</strong><p>Recent reports help separate a true update from yesterday’s repeated headline.</p></div></li><li><span>07</span><div><strong>Draft, continue, or repair</strong><p>The writer creates the briefing. Incomplete or invalid work is edited and checked again.</p></div></li><li><span>08</span><div><strong>Save, build, and test</strong><p>A protected Markdown file becomes HTML pages, a search record, and a deployable site artifact.</p></div></li></ol></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">THE DATA CHANGES CLOTHES</div><h2>One story, five useful shapes.</h2></div><p>Each station repacks the same evidence for the next job.</p></div><div class="architecture-data-flow"><div><span>📡</span><strong>Feed item</strong><small>A title, link, time, and source.</small></div><b aria-hidden="true">→</b><div><span>🧺</span><strong>Story cluster</strong><small>Matching clues bundled together.</small></div><b aria-hidden="true">→</b><div><span>🗂️</span><strong>Research pack</strong><small>Ranked evidence and continuity notes.</small></div><b aria-hidden="true">→</b><div><span>📝</span><strong>Markdown</strong><small>The checked human-readable report.</small></div><b aria-hidden="true">→</b><div><span>🖥️</span><strong>HTML + index</strong><small>Fast pages and browser search data.</small></div></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">TWO DIFFERENT ROOMS</div><h2>Build-time work versus browser-time work.</h2></div></div><div class="architecture-worlds"><article><span class="architecture-world-label">BEFORE YOU VISIT</span><h3>🔧 The workshop</h3><p>Research, writing, repair, validation, Markdown saving, page building, tests, and packaging happen here.</p><ul><li>Can use private automation secrets</li><li>Produces public static files</li><li>Stops safely when a required check fails</li></ul></article><article><span class="architecture-world-label">AFTER YOU VISIT</span><h3>📖 The reading room</h3><p>Your browser receives ready-made files and handles small optional interactions locally.</p><ul><li>Shows HTML styled by CSS</li><li>Searches a small downloaded index</li><li>Never receives private provider credentials</li></ul></article></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-section-heading"><div><div class="eyebrow">WHEN THINGS GO WRONG</div><h2>The safety net has several knots.</h2></div><p>A failure should leave the last good public site standing.</p></div><div class="architecture-failures"><article><span>📵</span><h3>A source is offline</h3><p>Other usable sources can continue. No response is treated as missing evidence, not permission to invent.</p></article><article><span>⏳</span><h3>Writing times out</h3><p>Retries stay within limits, then a conservative evidence-only edition can preserve the day.</p></article><article><span>🩹</span><h3>The draft has mistakes</h3><p>The exact problems are sent back for editing. The edited draft must pass validation again.</p></article><article><span>🧱</span><h3>Build or tests fail</h3><p>No successful site artifact is published, so visitors keep seeing the previous good version.</p></article></div></section><section class="architecture-section page-container" data-reveal><div class="architecture-example"><div class="architecture-example-icon">🤖</div><div><div class="eyebrow">A WORKED EXAMPLE</div><h2>Suppose a lab announces a school-helper robot.</h2><p>The official announcement, a technical paper, and a discovery article enter through three doors. Clustering recognizes one event. The official page and paper become strong evidence; the discovery article becomes context. The writer explains what is demonstrated, labels what is only claimed, and avoids inventing a price. Validation checks the links and required sections. One careful story then appears in Markdown, HTML, the archive, and search.</p><strong>Three doors in → one verified story out.</strong></div></div></section>`;
  return architectureSummaryBody()
    .replace('How this little AI newsroom works.', 'See how this little AI newsroom works down below.')
    .replace('<section class="architecture-next page-container"', `${deepDive}<section class="architecture-next page-container"`);
}
...
const architecturePage = siteShell({ title: 'How It Works', description: 'A friendly guide to the Daily AI Intelligence architecture.', body: architectureBody(), active: 'architecture' });
await writeFile('how-it-works/index.html', architecturePage);
// Preserve the original public URL for bookmarks and older links.
await writeFile('architecture/index.html', architecturePage);

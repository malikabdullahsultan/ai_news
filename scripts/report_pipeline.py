...
def build_evidence_fallback_report(report_date: date, research: dict[str, Any]) -> str:
    ...
    if not stories:
        ...
        lines = [
            ...
            "",
            "This status edition is the safety mechanism: it preserves the Hong Kong calendar date, makes the lack of evidence visible, and gives the next scheduled retry an idempotent target. In plain English: the newsroom robot found an empty evidence tray and wisely chose not to improvise. It is not evidence that no AI activity occurred; it means only that this pipeline could not verify activity from its configured inputs during this run.",
            "",
